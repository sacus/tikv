...
pub fn flush_engine_ticker_metrics(t: TickerType, value: u64) {
    match t {
        ...
        TickerType::BlockCacheByteRead => {
            STORE_ENGINE_CACHE_EFFICIENCY_VEC.with_label_values(&["block_cache_byte_read"])
                .set(value as f64);
        }
        TickerType::BlockCacheByteWrite => {
            STORE_ENGINE_CACHE_EFFICIENCY_VEC.with_label_values(&["block_cache_byte_write"])
                .set(value as f64);
        }
        TickerType::MemtableHit => {
        ...
        TickerType::FlushWriteBytes => {
            STORE_ENGINE_FLOW_VEC.with_label_values(&["flush_write_bytes"])
                .set(value as f64);
        }
        TickerType::WalFileBytes => {
            STORE_ENGINE_FLOW_VEC.with_label_values(&["wal_file_bytes"])
                .set(value as f64);
        }
        TickerType::StallMicros => {
        ...
        TickerType::CompactionRangeDelDropObsolete => {
            STORE_ENGINE_COMPACTION_DROP_VEC
                .with_label_values(&["compaction_range_del_drop_obsolete"])
                .set(value as f64);
        }
        TickerType::NumberDbSeek => {
            STORE_ENGINE_LOCATE_VEC.with_label_values(&["num_db_seek"])
                .set(value as f64);
        }
        TickerType::NumberDbNext => {
            STORE_ENGINE_LOCATE_VEC.with_label_values(&["num_db_next"])
                .set(value as f64);
        }
        TickerType::NumberDbPrev => {
            STORE_ENGINE_LOCATE_VEC.with_label_values(&["num_db_prev"])
                .set(value as f64);
        }
        TickerType::NumberDbSeekFound => {
            STORE_ENGINE_LOCATE_VEC.with_label_values(&["num_db_seek_found"])
                .set(value as f64);
        }
        TickerType::NumberDbNextFound => {
            STORE_ENGINE_LOCATE_VEC.with_label_values(&["num_db_next_found"])
                .set(value as f64);
        }
        TickerType::NumberDbPrevFound => {
            STORE_ENGINE_LOCATE_VEC.with_label_values(&["num_db_prev_found"])
                .set(value as f64);
        }
        TickerType::NoFileCloses => {
            STORE_ENGINE_FILES_STATUS_VEC.with_label_values(&["no_file_closes"])
                .set(value as f64);
        }
        TickerType::NoFileOpens => {
            STORE_ENGINE_FILES_STATUS_VEC.with_label_values(&["no_file_opens"])
                .set(value as f64);
        }
        TickerType::NoFileErrors => {
            STORE_ENGINE_FILES_STATUS_VEC.with_label_values(&["no_file_errors"])
                .set(value as f64);
        }
        TickerType::NoIterators => {
            STORE_ENGINE_NO_ITERATOR_VEC.with_label_values(&["no_iterators"])
                .set(value as f64);
        }
        TickerType::WalFileSynced => {
            STORE_ENGINE_WAL_FILE_SYNCED_VEC.with_label_values(&["wal_file_synced"])
                .set(value as f64);
        }
        TickerType::ReadAmpEstimateUsefulBytes => {
            STORE_ENGINE_READ_AMP_VEC.with_label_values(&["read_amp_estimate_useful_bytes"])
                .set(value as f64);
        }
        TickerType::ReadAmpTotalReadBytes => {
            STORE_ENGINE_READ_AMP_VEC.with_label_values(&["read_amp_total_read_bytes"])
                .set(value as f64);
        }
    }
}
...
lazy_static!{
    ...
    pub static ref STORE_ENGINE_WAL_FILE_SYNCED_VEC: GaugeVec =
        register_gauge_vec!(
    ...
    pub static ref STORE_ENGINE_COMPACTION_DROP_VEC: GaugeVec =
        register_gauge_vec!(
            "tikv_engine_compaction_drop",
            "Number of key drop during compaction",
            &["type"]
        ).unwrap();
    ...
    pub static ref STORE_ENGINE_FILES_STATUS_VEC: GaugeVec =
        register_gauge_vec!(
            "tikv_engine_files_status",
            "Number of files status.",
            &["type"]
        ).unwrap();

    pub static ref STORE_ENGINE_NO_ITERATOR_VEC: GaugeVec =
        register_gauge_vec!(
    ...
    pub static ref STORE_ENGINE_READ_AMP_VEC: GaugeVec =
        register_gauge_vec!(
            "tikv_engine_read_amp",
            "Total bytes of read amplification.",
            &["type"]
        ).unwrap();
}
